...
void main() {
	...
	do {
		...
		printf("15: Сохранить состояние массива в файла\n");
		printf("16: Загрузить состояние массива из файла\n");
		printf("\n");
		...
		{
			FILE* fout;
			fout = fopen("D:\\Stadies\\Labs\\17out.txt", "wt");
			if (fout == NULL) {
			...
			printf("сохранили массив arr[%d] = ", n);
			for (int i = 0; i < n; i++) {
					printf("%d ", arr[i]);
		...
		case 16:
		{
			FILE* fin = fopen("D:\\Stadies\\Labs\\17out.txt", "rt");
			if (fin == NULL) {
